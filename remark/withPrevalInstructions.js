      ...
      let information =
        outdatedVersions.length > 0
          ? `${joinAsSpeech(outdatedVersions)} ${
            ...
            } so you need to install [the BYODocs v2.0 PostCSS 7 compatibility build](/docs/installation#post-css-7-compatibility-build) for now as we've shown above.`
          : ''

      return md(`
        ## Setting up BYODocs

        *BYODocs requires Node.js 12.13.0 or higher.*
    ...
    finish({ scripts = [], tool = pageTool }) {
      return md(`
        You're finished! Now when you run ${joinAsSpeech(
          scripts.map(quote('`')),
          ' or '
        )}, BYODocs will be ready to use in your ${tool} project.
